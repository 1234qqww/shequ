<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:62:"D:\xy\wuliu\public/../application/admin\view\banner\slide.html";i:1560672025;}*/ ?>
...
<script>
    layui.config({
        ...
        var cols=[[
            ...
            ,{field: 'imgs',align: 'center', title: '图片',toolbar: '#img'}
            ,{field: 'display',align: 'center', title: '状态',toolbar: '#display'}
            ,{field: 'create_time',align: 'center', title: '添加时间', sort: true}
            ,{title: '操作', width: 150, align: 'center', fixed: 'right', toolbar: '#action'}
        ]];
        //加载数据
        base_table(table,'list','/admin/banenr/slide',cols);
        table.on('tool(list)',function(obj){
            var data=obj.data;
            if(obj.event=='del'){
                layer.confirm('确定删除此轮播图？', function(index){
                    var post_json={id:data.id};
                    hhtc.ajax('<?php echo url("company/del_banner"); ?>',post_json,function(res){
                        if(res.code==1){
                ...
                layer.open({
                    type: 2
                    ,title: '编辑轮播图'
                    ,content: '/admin/company/edit_banner/id/'+data.id
                    ,area: ['100%', '100%']
                    ,btn: ['确定', '取消']
                    ,yes: function(index, layero){
                        ...
                        iframeWindow.layui.form.on('submit(edit)', function(data){
                            hhtc.ajax("<?php echo url('company/edit_banner'); ?>",data.field,function(res){
                                if(res.code==1){
                                    layer.msg(res.msg,{icon:1},function(){
                                        table.reload('list');
                ...
                layer.open({
                    ...
                    shadeClose: true,
                    area:['768px','304px'],
                    content: '<img width="100%" src="'+data.img+'">'
                });
            }
        })
        //事件
        var active = {
            add: function(){
                layer.open({
                            type: 2
                            ,title: '添加轮播图'
                            ,content: "<?php echo url('company/add_banner'); ?>"
                        ,area: ['100%', '100%']
                        ,btn: ['确定', '取消']
                        ,yes: function(index, layero){
                    var iframeWindow = window['layui-layer-iframe'+ index]
                    ...
                    iframeWindow.layui.form.on('submit(add)', function(data){
                        ...
                        hhtc.ajax("<?php echo url('company/add_banner'); ?>",data.field,function(res){
                            if(res.code==1){
                                layer.msg(res.msg,{icon:1},function(){
                                    table.reload('list');
                                    layer.close(index); //关闭弹层
