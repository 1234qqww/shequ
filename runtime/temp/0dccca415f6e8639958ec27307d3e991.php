<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:93:"D:\xy\project\shequshop\public/../application/admin\view\commodity\commodity_jifen_class.html";i:1565678933;}*/ ?>
        ...
        table.on('tool(list)',function(obj){
            ...
            if(obj.event=='del'){
                layer.confirm('确定删除此分类？', function(index){
                    ...
                    hhtc.ajax('<?php echo url("commodity/commodity_jifen_del"); ?>',post_json,function(res){
                        if(res.code==1){
                ...
                layer.open({
                    ...
                    ,yes: function(index, layero){
                        ...
                        iframeWindow.layui.form.on('submit(edit)', function(data){
                            hhtc.ajax("<?php echo url('commodity/commodity_jifen_edit'); ?>",data.field,function(res){
                                if(res.code==1){
                                    layer.msg(res.msg,{icon:1},function(){
                                        table.reload('list');
        ...
        var active = {
            add: function(){
                layer.open({
                    type: 2
                    ,title: '添加视频分类'
                    ,content: "<?php echo url('commodity/commodity_jifen_add'); ?>"
                    ,area: ['40%', '40%']
                    ,btn: ['确定', '取消']
                    ,yes: function(index, layero){
                        var iframeWindow = window['layui-layer-iframe'+ index]
                        ...
                        iframeWindow.layui.form.on('submit(add)', function(data){
                            ...
                            hhtc.ajax("<?php echo url('commodity/commodity_jifen_add'); ?>",data.field,function(res){
                                if(res.code==1){
                                    layer.msg(res.msg,{icon:1},function(){
                                        table.reload('list');
                                        layer.close(index); //关闭弹层
