<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:75:"D:\xy\project\wuliu\public/../application/admin\view\merchant\merchant.html";i:1560837270;}*/ ?>
...
<div class="layui-fluid">
    <div class="layui-card">
        <div class="layui-card-body">
            ...
            <script type="text/html" id="merportrait">
                <img src="{{d.merportrait}}"  lay-event="showimg" alt="图片丢失">
            </script>
            <script type="text/html" id="shifa">
                {{d.prov}}{{d.city}}{{d.area}}{{d.addresss}}
            </script>
            <script type="text/html" id="zhida">
                {{d.setout}}->{{d.arrive}}
            </script>
            <script type="text/html" id="xiehuo">
                {{d.provs}}{{d.citys}}{{d.areas}}{{d.addresss}}
            </script>
...
<script>
    layui.config({
        ...
        var cols = [[
            ...
            , {field: 'mername',width: 80, align: 'center', title: '商户名称'}
            , {field: 'merportrait',width: 120, align: 'center', title: '商户头像',toolbar: '#merportrait'}
            , {field: 'contacts',width: 80, align: 'center', title: '联系人'}
            , {field: 'telephone', align: 'center', title: '电话'}
            , {field: 'mobile', align: 'center', title: '手机'}
            , {align: 'center',width: 250,title: '始发地', toolbar: '#shifa'}
            , {align: 'center',width: 150, title: '直达路线', toolbar: '#zhida'}
            , { align: 'center',width: 250, title: '卸货地址', toolbar: '#xiehuo'}
        ...
        table.on('tool(list)', function (obj) {
            var data = obj.data;
            if (obj.event == 'del') {
                layer.confirm('确定删除此商户？', function (index) {
                    var post_json = {id: data.id};
                    hhtc.ajax('<?php echo url("merchant/del_merchant"); ?>', post_json, function (res) {
                        if (res.code == 1) {
                ...
                layer.open({
                    type: 2
                    , title: '编辑商户'
                    , content: '/admin/merchant/edit_merchant/id/' + data.id
                    , area: ['100%', '100%']
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(edit)', function (data) {
                            ...
                            hhtc.ajax("<?php echo url('merchant/edit_merchant'); ?>", data.field, function (res) {
                ...
                layer.open({
                    ...
                    shadeClose: true,
                    area:['768px','304px'],
                    content: '<img width="100%" src="'+data.merportrait+'">'
                });
            }
