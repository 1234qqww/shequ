<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:81:"D:\xy\project\shequshop\public/../application/admin\view\banner\refund_state.html";i:1564563827;}*/ ?>
        ...
        table.on('tool(list)',function(obj){
            ...
            if(obj.event=='del'){
                layer.confirm('确定删除此状态？', function(index){
                    ...
                    hhtc.ajax('<?php echo url("banner/del_refund_state"); ?>',post_json,function(res){
                        if(res.code==1){
                ...
                layer.open({
                    ...
                    ,yes: function(index, layero){
                        ...
                        iframeWindow.layui.form.on('submit(edit)', function(data){
                            ...
                            hhtc.ajax("<?php echo url('banner/edit_refund_state'); ?>",data.field,function(res){
                                if(res.code==1){
                                    layer.msg(res.msg,{icon:1},function(){
                                        table.reload('list');
        ...
        var active = {
            add: function(){
                layer.open({
                    type: 2
                    ,title: '添加状态'
                    ,content: "<?php echo url('banner/add_refund_state'); ?>"
                    ,area: ['40%', '40%']
                    ,btn: ['确定', '取消']
                    ,yes: function(index, layero){
                        var iframeWindow = window['layui-layer-iframe'+ index]
                        ...
                        iframeWindow.layui.form.on('submit(add)', function(data){
                            ...
                            hhtc.ajax("<?php echo url('banner/add_refund_state'); ?>",data.field,function(res){
